<script lang="ts">
	...
	interface ApiStatus { label: string; key: string }
...
<header
	class="flex h-14 shrink-0 items-center justify-between border-b border-border bg-background/80 px-4 backdrop-blur-sm"
...
>
	...
	<div class="flex items-center gap-2">
		...
		<div class="flex items-baseline gap-1.5">
			...
			<span class="hidden text-sm font-medium text-foreground sm:inline">Climate Risk Dashboard</span>
	...
	<div class="hidden items-center gap-3 lg:flex">
		{#each apiStatuses as api}
			<div class="flex items-center gap-1">
				...
				></div>
				<span class="text-[10px] text-muted-foreground">{api.label}</span>
			</div>
		{/each}
	</div>
	...
	<div class="flex items-center gap-2">
		{#if data.lastUpdated}
			<span class="hidden font-mono text-[10px] text-muted-foreground xl:block">
				Updated {formatDateTime(data.lastUpdated)}
			</span>
		{/if}
		<ThemeToggle />
		<button
			class="flex h-8 w-8 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:bg-muted hover:text-foreground lg:hidden"
			onclick={onMenuClick}
