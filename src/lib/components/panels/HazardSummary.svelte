	...
	let hazards = $derived<HazardRow[]>([
		{
			emoji: '🔥',
			label: 'Fire Hazard',
			risk: fireCountToRisk(data.fires.length),
			source: 'NASA FIRMS'
		},
		{
			emoji: '💧',
		...
		},
		{
			...
			risk: data.drought ? droughtSeverityToRisk(data.drought.severity) : 'MEDIUM',
			source: 'Open-Meteo'
		},
		{
			emoji: '🌍',
			label: 'Seismic Background',
			risk: seismicCountToRisk(data.earthquakes.length),
			source: 'USGS'
		},
		{
			emoji: '👥',
		...
		{#each hazards as h}
			<div class="flex items-center justify-between gap-2">
				<div class="flex items-center gap-1.5 min-w-0">
					...
					<div class="min-w-0">
						<div class="text-xs font-medium text-[hsl(210_40%_98%)] truncate">{h.label}</div>
						<div class="text-[9px] text-[hsl(215_20%_40%)]">{h.source}</div>
					</div>
				</div>
